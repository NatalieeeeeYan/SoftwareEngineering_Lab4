                          ...
                          <div v-for="shop in openingShops" :key="shop">
                            ...
                            <div class="q-pa-md row items-start q-gutter-md" style="width: 80%;">
                              <q-card class=" my-card" flat bordered style="width: 80%;">
                                ...
                                <q-card-actions>
                                  <q-btn push icon="delete" @click="delete_shop_confirm = true" />

                                  <q-dialog v-model="delete_shop_confirm" persistent>
                                    <q-card>
                                      <q-card-section class="row items-center">
                                        <q-avatar icon="warning" color="negative" text-color="white" />
                                        <span class="q-ml-sm">确定要关闭店铺{{ shop.shopName }}吗？</span>
                                      </q-card-section>

                                      <q-card-actions align="right">
                                        <q-btn flat label="取消" color="indigo-10" v-close-popup />
                                        <q-btn flat label="确认" color="pink-13" v-close-popup @click="deleteShop(shop)" />
                                      </q-card-actions>
                                    </q-card>
                                  </q-dialog>
...
function toShop(shop) {
  ...
  console.log("shopId: ", store.state.shopId);

  localStorage.setItem('shopId', shop.id);

  if (shop.status == "闭店未审核" || shop.status == "开店已通过" || shop.status == "闭店未通过") {
...
function onSubmit() {
  ...
  console.log("申请开店提交表单：\ncurrent date: ", current_time.value, "\nfund: ", fund.value,
    "\nshopName: ", shopName.value, "\ncategory: ", newTextArr, "\nidNumber: ", idNumber.value,
    "\naddress: ", address.value, "\nregisteration time: ", registrationTime.value, "\nuser id: ".userId)
...
function deleteShop(shop) {
  ...
  axiosInstance.post('/shop/delete', {
    ...
    instance.proxy.$forceUpdate();
    if (code.value == 20000) {
      delete_shop_confirm.value = false
      operation_success_message.value = '您已申请关闭店铺', shop.shopName
      operation_success_alert_layout.value = true
      update()
    } else {
      delete_shop_confirm.value = false
      operation_fail_message.value = '申请闭店失败，', response.data['message']
      operation_success_alert_layout.value = true
      update()
    }
  }).catch((error) => {
